...
const LOG_CMDS = ["SYNCING...", "ENCRYPTING...", "SECTOR_7_SCAN", "UUID_MATCH", "CORE_STABLE", "GATE_OPEN", "DECRYPT_RSA", "TRACE_CLEARED"];
let activeUser = null;
...
function initRadar() {
    ...
    const pts = [
        ...
        { x: 70, y: 45, n: "БУНКЕР P.R.I.S.M", c: "#00f7ff", act: true },
    ...
    pts.forEach(p => {
        ...
        d.style.boxShadow = `0 0 10px ${p.c}`;
        ...
        d.onclick = () => {
            ...
            document.getElementById('rad-text').innerText = p.act ? "СТАТУС: АКТИВЕН // СИНХРОНИЗАЦИЯ: 100% // СИГНАЛ: СТАБИЛЬНЫЙ" : "СТАТУС: OFFLINE // ПИТАНИЕ ПРЕРВАНО";
            // Плавный скролл вниз к инфо
            setTimeout(() => {
...
function renderArchive() {
    const data = [
        {id: "LOG_882", msg: "Ядро V32.8 функционирует в штатном режиме. Все протоколы активны."},
        {id: "LOG_885", msg: "Зафиксирована попытка сканирования портов из внешнего сектора."},
        {id: "LOG_890", msg: "Синхронизация UUID завершена. MSK4NE подтвержден."}
    ];
    ...
    list.innerHTML = data.map(i => `
        <div class="db-log-item">
            <div class="db-log-header" onclick="this.nextElementSibling.classList.toggle('open')">
                <span>[ ${i.id} ]</span> <span>ДАННЫЕ_ОБЪЕКТА</span>
            </div>
    ...
    setTimeout(() => {
        ...
        if(id === 'scr-guest') {
            ...
            guestInterval = setInterval(() => {
                ...
                box.prepend(l); if(box.childNodes.length > 12) box.removeChild(box.lastChild);
            }, 800);
        }
