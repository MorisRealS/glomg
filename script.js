...
const STAFF_QUOTES = [
    ...
    "Sumber ожидает поставку новых энергетических ядер.",
...
function checkMessages() {
    ...
    if(list) {
        ...
        `).reverse().join('') : "Входящих данных нет.";
...
function initializeTacticalRadar() {
    ...
    const nodes = [
        {id: "ONG_HQ", x: 48, y: 35, info: "ШТАБ ОНГ. Центральный узел."},
        {id: "SUMBER_BUNKER", x: 25, y: 25, info: "БУНКЕР SUMBER. Склад ядер."},
        {id: "PRISM_EYE", x: 75, y: 65, info: "БУНКЕР ПРИЗМЫ. Враждебная зона."},
        {id: "DYK_REACTOR", x: 40, y: 80, info: "РЕАКТОР DYK. Стабильно."}
    ];
    ...
    nodes.forEach(n => {
        ...
        d.onclick = () => {
            ...
            document.getElementById('radar-scroll').scrollTo({ top: 800, behavior: 'smooth' });
        };
        cont.appendChild(d);
    });
}
...
function playTypingSound() { sound_type.currentTime = 0; sound_type.play(); }
...
window.onload = () => {
    ...
    document.getElementById('intro-ascii').innerText = "SYSTEM_V32.8_READY";
    setTimeout(() => document.getElementById('intro-logo').classList.remove('hidden'), 1000);
};
